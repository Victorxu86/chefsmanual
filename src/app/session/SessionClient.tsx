...
export function SessionClient({ recipes }: { recipes: any[] }) {
  ...
  const lanes = useMemo(() => {
    ...
    // Oven (仅当需要或配置了数量 > 0 时显示，但为了让用户看到资源增加的效果，只要 resources.oven > 0 就显示)
                ...
                <div className="mt-12 space-y-6 pb-12">
                  {lanes.map(lane => (
                    <Lane 
                      key={lane.id} 
                      title={lane.title} 
                      // 关键修正：即使 blocks 为空也渲染 Lane，让用户感知到资源的增加
            ...
            return (
              <button
                ...
                className={`w-8 h-8 rounded flex items-center justify-center text-sm font-bold transition-all duration-200
                  ${isActive 
                    ? `${activeColorClasses[color]} text-white shadow-md transform scale-105` // 修复样式：确保选中态清晰
